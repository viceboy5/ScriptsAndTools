...
def cleanRibbon(ribbonNme, matrixRibbon):
    ...
    # Other Bits
    extraNodeList = (cmds.ls(ribbonNme + "_ikHandle_*") + cmds.ls(ribbonNme + "_tweak_*offset") + cmds.ls(
        ribbonNme + "_sine_*", tr=1) + cmds.ls(ribbonNme + "_twist_*", tr=1))
...
def convertToMatrix(*args):
    ...
    if not cmds.objExists(ribbonNme + "_follicle_01"): cmds.error(
        "Follicles not found, make sure the name of the ribbon is correct.")
...
def useRibbon(*args):
    ...
    if selRibbonList:
        ...
        for selRibbon in selRibbonList:
            ...
            if not cmds.attributeQuery("jointOrientMenu", ex=1, n=selRibbon): cmds.error(
                "Unable to update this ribbon, please use one generated with this tool.")
            ...
            orientation = (cmds.getAttr(selRibbon + ".orientationX"), cmds.getAttr(selRibbon + ".orientationY"),
                           cmds.getAttr(selRibbon + ".orientationZ"))

            endorient = (cmds.getAttr(selRibbon + ".endorientX"), cmds.getAttr(selRibbon + ".endorientY"),
                         cmds.getAttr(selRibbon + ".endorientZ"))
    ...
    for i in range(1, ribbonSpans):
        ...
        if i == spanNumber:
            ...
            if controlGroup:
                ...
                cmds.group(jointName.replace("joint", "control") + "_offset",
                           n=jointName.replace("joint", "control") + "_driver")
            ...
            cmds.move(moveAmount[0], moveAmount[1], moveAmount[2], jointName.replace("joint", "control") + ".cv[0:11]",
                      r=1, os=1)
...
def buildCustomControls(ribbonNme, jntList, genRibbon, matrixRibbon, ribbonDiv, quadLimbInput):
    ...
    for i in range(1, ribbonDiv + 1):
        if i == jointNumber or i == ribbonDiv:
            ...
            dupeJoint((ribbonNme + "_joint" + buffer), (ribbonNme + "_joint" + buffer),
                      (ribbonNme + "_ctrljnt" + buffer), 0.1 * globalScale)
            ...
            if controlGroup:
                ...
                cmds.group((ribbonNme + "_control" + buffer + "_driver"),
                           n=(ribbonNme + "_control" + buffer + "_offset"))
            else:
                cmds.group((ribbonNme + "_control" + buffer), n=(ribbonNme + "_control" + buffer + "_offset"))

            cmds.matchTransform((ribbonNme + "_control" + buffer + "_offset"), (ribbonNme + "_ctrljnt" + buffer), pos=1,
                                rot=1)
            ...
            if genRibbon: cmds.move(moveAmount[0], moveAmount[1], moveAmount[2],
                                    (ribbonNme + "_control" + buffer + ".cv[0:11]"), r=1, os=1)
    ...
    # Bind
    cmds.skinCluster(ribbonNme, (cmds.ls(((ribbonNme + "_ctrljnt_*"), (ribbonNme + "_angle_*")), type="joint")), tsb=1,
                     dr=10, n=(ribbonNme + "_skinCluster"))
...
def addAngleControls(ribbonNme, cntrlJointList, quadLimbInput, controlOri, *args):
    ...
    for i in range(len(cntrlJointList) - 1):
        dupeJoint(cntrlJointList[i], cntrlJointList[i], (cntrlJointList[i].replace("_ctrljnt_", "_angle_")),
                  0.25 * globalScale)
        dupeJoint(cntrlJointList[i], cntrlJointList[i + 1], (cntrlJointList[i].replace("_ctrljnt_", "_angletip_")),
                  0.25 * globalScale)

        cmds.parent((cntrlJointList[i].replace("_ctrljnt_", "_angletip_")),
                    (cntrlJointList[i].replace("_ctrljnt_", "_angle_")))
        cmds.parent((cntrlJointList[i].replace("_ctrljnt_", "_angle_")), cntrlJointList[i])

        cmds.ikHandle(n=(cntrlJointList[i].replace("_ctrljnt_", "_ikHandle_")), sol="ikRPsolver",
                      sj=(cntrlJointList[i].replace("_ctrljnt_", "_angle_")),
                      ee=(cntrlJointList[i].replace("_ctrljnt_", "_angletip_")))
...
def addTweakControls(ribbonNme, cntrlJointList, quadLimbInput, controlOri, *args):
    ...
    for i in range(tweakCount):
        ...
        cmds.pointConstraint(cntrlJointList[i], cntrlJointList[i + 1], (ribbonNme + "_tweak_" + str(i) + "_offset"),
                             mo=0)

        cmds.aimConstraint(cntrlJointList[i], (ribbonNme + "_tweak_" + str(i) + "_aim"), w=1, aim=controlOri,
                           u=controlUp, wu=controlUp, wut="objectrotation", wuo=(cntrlJointList[i]), mo=0)
...
def polyRibbon(*args):
    ...
    if normalFlip:
        cmds.loft("tempinnercurve", "tempoutercurve", r=0, ch=0, u=1, c=0, ar=1, d=1, ss=1, rn=0, po=0, rsn=1,
                  n=ribbonNme)
    else:
        cmds.loft("tempinnercurve", "tempoutercurve", r=0, ch=0, u=1, c=0, ar=1, d=1, ss=1, rn=0, po=0, rsn=1,
                  n=ribbonNme)
...
def antcgiRibbonUI():
    ...
    # Create Window
    window = cmds.window("antcgiRibbonTool", t="Ribbon Tool v2.8", w=225, mnb=0, mxb=0, s=1,
                         cc=partial(antcgiStoreRibbonUI, 1))

    # Create a main layout
    mainLayout = cmds.formLayout("antcgiRibbonForm", numberOfDivisions=100)

    # Create the news frame
    newsFrame = cmds.frameLayout("newsFrame", label='News & Updates', cll=1, cl=0, bv=0,
                                 cc=partial(cmds.window, "antcgiRibbonTool", e=1, h=200))
    newsLayout = cmds.formLayout("newsLayout", numberOfDivisions=100, bgc=(0.2, 0.2, 0.2))

    # Feed
    antcgiRibbonNews = cmds.scrollField("antcgiRibbonNews", ed=0, ww=1, h=100, bgc=(0.1, 0.1, 0.1),
    ...
    # Buttons
    button01 = cmds.button("polyRibbonButton", h=25, l="Generate Ribbon",
                           ann="Generate the ribbon using the guide curves.", p=mainLayout, c=polyRibbon, en=0)
    button02 = cmds.button("polyCustomButton", h=25, l="Build Custom Ribbon", p=mainLayout, c=custRibbon)

    button03 = cmds.button("outerButton", h=25, l="Outer Edges", ann="Create the outer guide curve.", p=mainLayout,
                           c=partial(storeEdges, "outer"))
    button04 = cmds.button("innerButton", h=25, l="Inner Edges", ann="Create the inner guide curve.", p=mainLayout,
                           c=partial(storeEdges, "inner"))

    button05 = cmds.button("convMatrix", h=25, l="Convert Follicles > Matrix", ann="Convert follicles to matrix nodes.",
                           p=mainLayout, c=convertToMatrix)
    button06 = cmds.button("updateRibbon", h=25, l="Update Selected Ribbon",
                           ann="Rebuild the follicles and controls on the selected ribbon.", p=mainLayout, c=useRibbon)

    # Ratio Menu
    controlRatioLabel = cmds.text("controlRatioLabel", label="Control Ratio", align="left", h=20)
    controlRatioMenu = cmds.optionMenu("controlRatioMenu", h=20,
                                       ann="How many joints do you want between each control?")
    ...
    ribbonDirMenu = cmds.optionMenu("ribbonDirMenu", h=20, w=104,
                                    ann="Do you want the ribbon to be horizontal or vertical?")
    ...
    ribbonOrientMenu = cmds.optionMenu("ribbonOrientMenu", w=20, h=20,
                                       ann="The orientation of the ribbon using the world axis.")
    ...
    # Checkbox
    jointOrientFlip = cmds.checkBox("jointOrientFlip", l="Invert", h=20,
                                    ann="Point the axis towards the ribbon instead of away?", v=0)
    flipRibbonNormals = cmds.checkBox("flipRibbonNormals", l="Flip Normals", h=15,
                                      ann="Flips the ribbon so the normals point the opposite way.", v=0)
    reduceRibbonSpans = cmds.checkBox("reduceRibbonSpans", l="Simplify Ribbon", h=15,
                                      ann="Halves the amount of spans in the ribbon.", v=0)
    ...
    contGroupInput = cmds.checkBox("contGroupInput", l="", h=15,
                                   ann="Add an extra group to help with additional controls?", v=0)
    ...
    angleControlsInput = cmds.checkBox("angleControlsInput", l="", h=15, ann="Add angle controls to the ribbon?", en=0,
                                       v=0)
    ...
    mtrxRibbonInput = cmds.checkBox("mtrxRibbonInput", l="", h=15,
                                    ann="Use matrix connections instead of follicles? [Maya 2020+ Only]", v=0)

    quadLimbLabel = cmds.text("quadLimbLabel", label="Three Joint Limb", align="left", h=15)
    quadLimbInput = cmds.checkBox("quadLimbInput", l="", h=15,
                                  ann="Create a ribbon for a limb with three joints (Dog Leg).", v=0)

    resetJntLabel = cmds.text("resetJntLabel", label="Reset End Joint", align="left", h=15)
    resetJntInput = cmds.checkBox("resetJntInput", l="", h=15,
                                  ann="Makes the end joint match the world space orientation.", v=0)
    ...
    globalScaleInput = cmds.floatField("globalScaleInput", h=20, w=50, pre=2,
                                       ann="Overall size of the scene - Use to make the ribbon and joints larger or smaller.",
                                       v=defValue)
    ...
    controlOffsetInput = cmds.floatField("controlOffsetInput", h=20, w=50, pre=2,
                                         ann="The distance the control is away from the joints.", v=0)
    ...
    cmds.formLayout(mainLayout, edit=True,
                    ...
                    attachControl=[(separator01, 'top', 5, newsFrame),
                                   ...
                                   (ribbonOrientMenu, 'top', 5, ribbonDirLabel),
                                   (ribbonOrientMenu, 'left', 18, ribbonOrientLabel),

                                   (jointOrientLabel, 'top', 5, ribbonOrientLabel),

                                   (jointOrientMenu, 'top', 5, ribbonOrientLabel),
                                   (jointOrientFlip, 'top', 5, ribbonOrientLabel),
                                   (jointOrientFlip, 'left', 10, jointOrientMenu),
